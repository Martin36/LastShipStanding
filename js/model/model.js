var model = function() {
  ...
  this.addBot = function(name, img) {
    var bot = new Player();
    if (!name) { //undefined
      bot.setName(getRandomName());
    } else {
      bot.setName(name);
    }
    bot.setImage(img);
    bot.setPosition(startPos[bots.length + players.length]);
    ...
    attackState.setNextState(avoidState);
    avoidState.setNextState(attackState);
    bot.fsm.addState(attackState);
    bot.fsm.addState(avoidState);
    bot.fsm.setState(attackState);
  ...
  this.addPlayer = function(name, img) {
    ...
    player.setImage(img);
    player.setPosition(startPos[bots.length + players.length]);
  ...
  this.update = function(dt) {
    ...
    for (var i = 0; i < bots.length; i++) {
      bots[i].updatePosition(windVelocity, dt);
    }
